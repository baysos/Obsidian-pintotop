...
- Menu labels and notices automatically use Chinese in Chinese Obsidian environments and English otherwise.
...
- 菜单和通知会自动判断 Obsidian 当前语言：中文环境显示中文，非中文环境显示英文。
- 命令面板提供清空所有置顶项的命令。
...
The GitHub release tag must match the `version` field in `manifest.json`, for example `1.0.5`.
